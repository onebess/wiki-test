...
import (
	...
	"github.com/onebess/wiki-test/utils"
...
func (page *WikipediaPage) GetSectionTreeList() ([]utils.WikiMenuTreeList, error) {
	if len(page.SectionTreeList) > 0 {
		return page.SectionTreeList, nil
	}
	args := map[string]string{
		"action": "parse",
		"prop":   "sections",
	...
	res, err := utils.WikiRequester(args)
	if err != nil {
		return []utils.WikiMenuTreeList{}, err
	}
	if res.Error.Code != "" {
		return []utils.WikiMenuTreeList{}, errors.New(res.Error.Info)
	}
	value := utils.SectionToTree(res.Parse["sections"].([]interface{}), "", 1)

	page.SectionTreeList = value
	return value, nil
}
...
func (page *WikipediaPage) GetSectionList() ([]string, error) {
	...
	tmp := res.Parse["sections"].([]interface{})
	fmt.Println(tmp)
